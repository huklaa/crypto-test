...
async function ensureRobinhoodChain(provider=activeProvider){if(!isProvider(provider))throw new Error("No browser wallet detected.");const current=await provider.request({method:"eth_chainId"});if(Number(current)===4663)return;try{await provider.request({method:"wallet_switchEthereumChain",params:[{chainId:CHAIN_ID_HEX}]})}catch(error){if(error?.code!==4902)throw error;await provider.request({method:"wallet_addEthereumChain",params:[{chainId:CHAIN_ID_HEX,chainName:"Robinhood Chain",nativeCurrency:{name:"Ether",symbol:"ETH",decimals:18},rpcUrls:[RPC_URL],blockExplorerUrls:[EXPLORER]}]})}}
...
function bindProviderEvents(provider){if(!provider?.on||provider.__chainlingBound)return;provider.__chainlingBound=true;provider.on("accountsChanged",accounts=>{account=accounts?.[0]??null;updateWalletUi();showMessage(account?`Wallet connected. Design #${selectedDesign} is ready to mint.`:"Wallet disconnected.")});provider.on("chainChanged",chainId=>{showMessage(Number(chainId)===4663?`Robinhood Chain connected. Design #${selectedDesign} is ready to mint.`:"Please switch to Robinhood Chain (4663).")})}
