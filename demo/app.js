...
function updateWalletUi(){const label=account?shortAddress(account):"Connect Wallet";connectButtons.forEach(b=>b.textContent=label);if(walletStatus)walletStatus.textContent=account?shortAddress(account):"Not connected"}
...
async function connectWallet(){try{await ensureRobinhoodChain();const accounts=await window.ethereum.request({method:"eth_requestAccounts"});account=accounts?.[0]??null;updateWalletUi();if(mintMessage)mintMessage.textContent=account?`Wallet connected. Design #${selectedDesign} selected at ${DESIGN_PRICES[selectedDesign]} USDG.`:"Wallet connection was not completed."}catch(error){if(mintMessage)mintMessage.textContent=error?.message||"Unable to connect wallet."}}
connectButtons.forEach(b=>b.addEventListener("click",connectWallet));
document.querySelectorAll(".nft-card").forEach(card=>card.addEventListener("click",()=>{document.querySelectorAll(".nft-card").forEach(i=>i.classList.remove("active"));card.classList.add("active");selectedDesign=Number(card.dataset.design);if(selectedName)selectedName.textContent=card.dataset.name;if(selectedSupply)selectedSupply.textContent=card.dataset.supply;renderSelectedPrice();syncHero();if(mintMessage)mintMessage.textContent=account?`Design #${selectedDesign} selected at ${DESIGN_PRICES[selectedDesign]} USDG.`:"Connect your wallet to prepare minting."}));
...
renderCardPrices();renderSelectedPrice();syncHero();updateWalletUi();
