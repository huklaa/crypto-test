...
async function finishWalletConnection(provider){await ensureRobinhoodChain(provider);const accounts=await provider.request({method:"eth_requestAccounts"});account=accounts?.[0]??null;activeProvider=provider;updateWalletUi();showMessage(account?`Wallet connected. Design #${selectedDesign} is ready to mint.`:"Wallet connection was not completed.");bindProviderEvents(provider)}
function bindProviderEvents(provider){if(!provider?.on||provider.__chainlingBound)return;provider.__chainlingBound=true;provider.on("accountsChanged",accounts=>{account=accounts?.[0]??null;updateWalletUi()});provider.on("chainChanged",()=>window.location.reload())}
async function connectWallet(){try{let candidate=activeProvider?{provider:activeProvider,info:activeProviderInfo}:choosePreferredInjected();if(!candidate){showWalletChooser();showMessage("Choose a wallet to continue.");return}activeProvider=candidate.provider;activeProviderInfo=candidate.info;await finishWalletConnection(candidate.provider)}catch(error){showMessage(errorMessage(error))}}
